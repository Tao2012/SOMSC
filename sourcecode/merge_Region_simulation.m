function L =  merge_Region_simulation(L,threshold,map)
...
mergePair = [];
adjacent_time = zeros(size(adjacent));
for i = 1:size(adjacent,1)
    ...
    adjacent_time(i,:) = [temp1,temp2];
end

for i = 1:max(max(adjacent_time))
   index1 = adjacent_time == i;
   index2 = index1(:,1) == 1 & (index1(:,1)-index1(:,2)) == 0;
   temp = minVline(index2); 
   quantile(temp,0.5)
   index3 = index2 & minVline < quantile(temp,0.5);
   mergePair = [mergePair;adjacent(index3,:)];
end
mergePair
