function map = addTextBarrier_ttest(map,L,mat,sData,pValue)
...
for i = 1:size(adjacent,1)
    ...
    [h,~] = ttest2(data1,data2,'Alpha',pValue);
    %     tempD = norm(data1_mean - data2_mean);
    if sum(~isnan(h)) == 0
        minVline(i) = 10000;
    else
        minVline(i) = sum(h(~isnan(h)));
    end
    ...
    text(tempCol(ind), tempRow(ind), sprintf('%d',minVline(i)), ...
